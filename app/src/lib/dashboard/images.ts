...
export type ImageFreshness = {
  ...
  error: string | null
  /**
   * The pin has moved to a different tag since this row was probed.
   *
   * Computed by `reconcileFreshness`, never read from the file. It is the
   * difference between "the registry says your tag has not moved" and "the
   * registry was asked about a tag you are no longer on" — and a panel that
   * states the first when the second is true is the confidently-wrong answer
   * this whole module is organised against.
   */
  stale: boolean
}
...
const freshnessShape = recordOf(
  obj({
    ...
    checkedAt: str,
    error: nullable(str),
    // Never written by the probe — reconcileFreshness computes it against the
    // live pin. Decoded with a default so the shape stays total.
...
/**
 * The probe's answers, reconciled against the pin as it stands NOW.
 *
 * The two clocks are the whole reason this exists. The probe runs daily; the
 * pin is regenerated by nix on every switch — so the moment an update lands,
 * every verdict in the probe's row is about a ref the box has left behind, and
 * the row keeps asserting it for up to a day.
 *
 * The symptom is not subtle and it is not theoretical: a freshly-updated
 * wealthfolio rendered as `3.6.3 → 3.6.3, newer tag`, offering to move a pin
 * to the version it had just been moved to. Both halves were the same mistake
 * — a probe answer trusted past the event that spent it.
 *
 * Where they disagree the PIN wins, because it is the live fact and the probe
 * is a recollection. What survives reconciliation is only what is still true:
 *
 *   - `moved` requires the probe to have asked about the tag we are actually
 *     on. Once the pin moves to a different tag, the remote digest recorded
 *     here belongs to the OLD tag and says nothing about this one.
 *   - `newerTag` is answered from `candidates`, which is a list of tags in the
...
export async function imageFreshness(container: string): Promise<ImageFreshness | null> {
  const raw = (await cachedFreshness())?.[container] ?? null
...
export function reconcileFreshness(
  f: ImageFreshness,
  pin: { tag: string; digest: string },
): ImageFreshness {
  // Everything the probe recorded about movement was asked of `f.tag`.
  const sameRef = f.tag === pin.tag

  const moved =
    sameRef && f.error === null && f.remoteDigest !== null && f.remoteDigest !== pin.digest

  const at = f.candidates.indexOf(pin.tag)
  const newerTag =
    at === 0
      ? // Top of its own shape — the answer regardless of which tag the probe
        // happened to ask about.
        null
      : at > 0
        ? (f.candidates[0] ?? null)
        : // Not in the list at all: an operator-chosen tag the probe has not
          // seen, or a channel pin with no candidates. The only claim still
          // checkable is the trivial one.
          f.newerTag === pin.tag
          ? null
          : f.newerTag

  return { ...f, pinnedDigest: pin.digest, moved, newerTag, stale: !sameRef }
}
