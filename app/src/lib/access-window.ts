// The time windows the access tab offers, split out from host/access.ts because
// the route imports them as VALUES (the range picker, validateSearch) and
// host/access.ts is server-only. Same split, and the same reason, as
// ./env-groups vs host/env-snapshot.ts.
...
> = {
  ...
  // Loki's retention is 30 days (nix/modules/logging), so this is the whole of
