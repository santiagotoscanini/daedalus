    ...
    <>
      ...
      >
        ...
        <p className="board-foot">
          ...
          endpoint is empty and would report zero on a box with {num(d.rules)}. Severity is a label
          on the generated alert instance rather than on the rule, which is why only active ones
          carry it.
        </p>
      </Board>
  ...
  return (
    <>
      ...
      <Board title="Certificates" icon="⚿" span={4}>
        ...
        />
        <p className="board-foot">
          One entrypoint-level wildcard covers <span className="mono">*.toscanini.me</span>, so this
          is effectively one certificate for every hostname on the box. Renewal is DNS-01 through
      ...
      <LogBoard
        ...
        foot={
          <p className="board-foot">
            ...
            served yet, which is why it is one of the containers gated behind a bounded probe of the
            real discovery URL rather than merely ordered after Pocket ID — see{' '}
    ...
    <>
      ...
      <Board title="Storage" icon="▦" span={4}>
        <Facts
          rows={[
            ...
            { k: 'Retention', v: d.retention.days === null ? DASH : `${num(d.retention.days)} days` },
            {
              k: 'Oldest sample',
              v: d.retention.oldestDays === null ? DASH : `${num(d.retention.oldestDays)} days back`,
            },
          ]}
        />
        <p className="board-foot">
          The two retention rows are the reading: if the oldest sample is well short of the window,
          the disk cap bit before the time limit did and the history is shorter than configured.
        </p>
      </Board>
      ...
      <Board title="Series, seven days" icon="◫" span={8}>
        ...
        <p className="board-foot">
          ...
          down is a new label with unbounded values — the way a TSDB usually gets into trouble, and
          the thing a total sample count would not show.
        </p>
      </Board>
    ...
    <>
      ...
      >
        ...
        <p className="board-foot">
          ...
          nothing on its own, while the error line is the one worth a glance. A spike in the second
      ...
      >
        ...
        <p className="board-foot">
          ...
          own name. The <span className="mono">adhoc</span> bucket is the exception worth watching —
          it catches containers started by hand rather than by a unit, which once minted 77 phantom
    ...
    <>
      ...
      >
        ...
        <p className="board-foot">
          ...
          healthchecks actually knows. The two are different guarantees:{' '}
          <b>mail on failure</b> means a run that fails tells you, and <b>pinging</b> means a run
          that stops happening at all tells you. Only the second catches a timer that was disabled,
          never fired, or whose service was renamed — which is the failure a scheduled job actually
          has. {num(d.emailOnly)} of the {num(d.jobs.length)} here are mail-only, deliberately: for
          a job that runs on every rebuild, &ldquo;it did not run&rdquo; is not a fault.
        </p>
      </Board>
