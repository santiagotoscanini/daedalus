...
let
  ...
  secretsScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # An array rather than a literal word list: with one key declared
      # the list is one word, and shellcheck reads that as a quoting
      # mistake (SC2043) and fails the build.
      NAMES=(${lib.concatStringsSep " " (map secretKey (lib.attrNames cfg))})

      for name in "''${NAMES[@]}"; do
        # An EMPTY value counts as missing — a truncated state file would
        # otherwise render a blank Authorization header into a consumer,
        # which the gateway rejects with the same 401 as a wrong key and
        # is considerably harder to recognise.
        if grep -q "^$name=." "$FILE"; then
          continue
        fi

        # `sk-` is not decoration: LiteLLM's own client libraries and a
        # good deal of OpenAI-compatible tooling assume the prefix.
      ...
      # Values for keys no longer declared are LEFT ALONE. Nothing reads
      # them, and keeping one means a consumer removed and re-added under
      # the same name comes back with the same credential rather than a
      # silent rotation. The gateway-side key outlives them too — drop it
      # in the admin UI if that is not what you want.
    '';
  };
