...
#     reachable via traefik (prometheus.<baseDomain> / grafana.<baseDomain>)
...
# the operator declares in the Claude Code MCP file the platform materializes —
...
#
# The host brings:
#   fleet.modules.monitoring.enable        the switch (default off, as every catalog module)
#   fleet.modules.monitoring.envSopsFile   Grafana's break-glass admin login
#   fleet.images.{prometheus,grafana,node-exporter}   the digest-pinned images
#   fleet.grafanaDashboardsByFolder.System.*          its own dashboards — the ones that
#                                          know its pools and layout (the engine ships the
#                                          generic set: cpu, memory, network, containers,
#                                          security, app access, about, container logs)
# Requires the shared cluster (Grafana's database) and the proxy; the embed
# policy for the control plane's panels appears only with the apps platform.
...
let
  cfg = config.fleet.modules.monitoring;
  ...
  # time. `user@1000.service` is the operator's systemd user manager; the inner
  # `user.slice` is podman's own default parent within it.
  cgroupRoot = "/sys/fs/cgroup/user.slice/user-${toString config.fleet.operator.uid}.slice/${config.fleet.operator.userService}/user.slice";

  # The router, and something past it. Two pings a minute is the whole
  # extent of what this house can learn about its own uplink: the router
  ...
  livenessScript = pkgs.writeShellScript "container-up-export" ''
    ...
    failed=$(systemctl --failed --plain --no-legend --no-pager | wc -l)
    {
      ...
      # accounting at a uniform path, and the operator owns it.
      #
      ...
      podman inspect --format '{{.ID}} {{.Name}} {{.State.Pid}} {{.HostConfig.NetworkMode}}' \
        ...
        # decrypted and the NIC encrypted. Summing both would double every
  ...
  #
  # The JSON is a TEMPLATE for four facts a dashboard cannot know: @hostName@
  # (the tag the "Dashboards" dropdown filters on, and the About title),
  # @configRepo@ (About's "provisioned from" path) and @ssoHost@ (security's
  # IdP panel) and @lanInterface@ (the NIC the network panels graph).
  # Substituted ONCE, here, over the assembled tree — so a stack's
  # byFolder contribution gets the same treatment as a static asset without
  # each readFile wrapping itself.
  dashboardsDir = pkgs.runCommand "grafana-dashboards" { } (
    ...
    + ''
      ...
      find $out -name '*.json' -print0 | while IFS= read -r -d "" f; do
        substituteInPlace "$f" \
          --replace-quiet '@hostName@' ${lib.escapeShellArg config.networking.hostName} \
          --replace-quiet '@configRepo@' ${lib.escapeShellArg config.fleet.config.repo} \
          --replace-quiet '@ssoHost@' ${lib.escapeShellArg (lib.removePrefix "https://" config.fleet.sso.issuerUrl)} \
          --replace-quiet '@lanInterface@' ${lib.escapeShellArg config.fleet.lanInterface}
      done
    ''
  );
  ...
  '';
  # The control plane's published entry, which the apps stack creates. It is
  # the one origin allowed to frame Grafana's panels; read guardedly so a host
  # without the apps platform simply frames nothing.
  controlPlane = config.fleet.webApps.daedalus or null;
in
{
  options.fleet.modules.monitoring = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
    ...
    envSopsFile = lib.mkOption {
      type = lib.types.path;
      example = lib.literalExpression "./host/sops/monitoring/env.sops";
      description = ''
        sops-encrypted dotenv carrying GF_SECURITY_ADMIN_USER and
        GF_SECURITY_ADMIN_PASSWORD — Grafana's break-glass login, which
        sits behind the SSO gate and is what the control plane
        authenticates to Grafana's API with. Host data: the engine carries
  ...
  config = lib.mkIf cfg.enable {
    # grafana admin credentials: the host's sops-encrypted dotenv, decrypted
    # to /run/secrets/grafana-env at activation.
    ...
    systemd.tmpfiles.rules = [
      "d ${textfileDir} 0755 ${config.fleet.operator.user} ${config.fleet.operator.group} -"
    ];
    ...
    # 1-min liveness sweep. Runs as the operator so it can talk to the rootless
    ...
    fleet.traefikRawRules."grafana-embed.yml" = lib.mkIf (controlPlane != null) ''
      http:
        middlewares:
          grafana-embed:
            headers:
    ...
    fleet.ssoClients.grafana = {
      ...
      launchURL = "https://${config.fleet.webApps.grafana.hostname}";
      callbackURLs = [ "https://${config.fleet.webApps.grafana.hostname}/login/generic_oauth" ];
      logoutCallbackURLs = [ "https://${config.fleet.webApps.grafana.hostname}/login/generic_oauth" ];
      consumers = [ "grafana" ];
      consumerEnv = {
    ...
    fleet.webApps = {
      ...
      grafana = {
        ...
        extraMiddlewares = lib.optional (controlPlane != null) "grafana-embed@file";
      };
    };
    ...
    virtualisation.oci-containers.containers.prometheus = mkRootlessContainer {
      image = pinnedImage "prometheus" "docker.io/prom/prometheus";
      ...
      extraOptions = [
        ...
        # nothing. Override to UID 0 → the operator, who owns the data dir.
        "--user=0:0"
    ...
    virtualisation.oci-containers.containers.grafana = mkRootlessContainer {
      image = pinnedImage "grafana" "docker.io/grafana/grafana";
      dependsOn = [ "prometheus" ];

      volumes = [
        "${config.fleet.stateRoot}/monitoring/grafana/data:/var/lib/grafana"
        # The relay password for GF_SMTP_PASSWORD__FILE (the platform's mail secret).
      ...
      environment = {
        ...
        GF_SERVER_ROOT_URL = "https://${config.fleet.webApps.grafana.hostname}";
        GF_SERVER_SERVE_FROM_SUB_PATH = "false";

        # Lets daedalus embed panels with /d-solo (the control plane's app
        ...
        # SMTP alert delivery via the same relay msmtp uses. Password
        # read from the bind-mounted mail secret through Grafana's __FILE
        # convention (grafana runs --user=0:0 → the operator, who owns it).
    ...
    # node-exporter runs in the host netns so it sees the real NIC
    ...
    virtualisation.oci-containers.containers.node-exporter = mkRootlessContainer {
      image = pinnedImage "node-exporter" "docker.io/prom/node-exporter";
