...
#   <machineState>/app-db/monitoring/env (machine-generated
...
#   (tmpfs, 0600, the operator's — podman injects it pre-userns-remap). A
...
let
  ...
  clusterEnv = "${config.fleet.machineState}/app-db/cluster/env";
  monEnv = "${config.fleet.machineState}/app-db/monitoring/env";
...
in
{
  config = lib.mkIf (config.fleet.modules.app-db.enable && enabled) {
    ...
    fleet.statePaths."${config.fleet.machineState}/app-db/monitoring".mode = "0700";
    ...
    # born on the box, kept in the machine state, ALTER ROLE re-syncs it every
    # run so rotation = delete the file + rebuild. (`monitoring` shares
    # the machine-state namespace with app databases — don't name an app
    # "monitoring".)
    systemd.services."app-db-monitoring-role" = {
      ...
      script = ''
        ...
        install -m 0600 -o ${config.fleet.operator.user} -g ${config.fleet.operator.group} /dev/stdin "${monEnv}" <<EOF
