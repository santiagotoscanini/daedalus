...
#
# The slug is a FULL owner/name, unlike the build bridge's bare app name: the
...
# Same split as power.sh, same reason: a request this agent correctly refuses (a
...
# request exits 1 rather than going through reject(): the app never writes
# one, so it is not a refusal of anything the app asked for.
...
REPO="$(jq -r '.repo // ""' <<<"$REQ_JSON")"
ACTOR="$(jq -r '.actor // "unknown"' <<<"$REQ_JSON")"
...
rm -f "$ERR"
as_operator mv -T -- "$TMP" "$DEST" || reject "$DEST appeared while cloning — the clone is left at $TMP"
