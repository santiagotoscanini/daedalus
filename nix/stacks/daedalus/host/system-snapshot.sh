...
# directory, so the publish (temp beside the target, rename) runs as the
# operator: host/lib.sh's rule that root never chmods, chowns or renames by
# name where someone else could plant a link.
write_json_atomic "$OUT_DIR/system.json" <"$tmp"
...
# page that could stop working with no way to see that it had. Counted from
# the private copy, not read back out of $OUT_DIR as root.
echo "published $("$JQ" '.disks | length' "$tmp") disks," \
  "$("$JQ" '.pools | length' "$tmp") pools," \
  "$("$JQ" '.datasets | length' "$tmp") datasets," \
  "$("$JQ" '.replication | length' "$tmp") replication pairs," \
  "$("$JQ" '.generations | length' "$tmp") generations," \
  "$("$JQ" '.failedUnits | length' "$tmp") failed units," \
  "$("$JQ" '.jobs | length' "$tmp") jobs"
