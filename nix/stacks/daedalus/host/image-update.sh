...
# edits it would otherwise have root copying the link's target into a commit
# that then gets pushed. Same binaries, same flags — the operator owns every
# file here, so `sed -i`'s temp-and-rename behaves exactly as it did as root.
flake_grep() {
  as_operator grep "$@"
}
flake_sed() {
...
while read -r m repo from_tag from_digest to_tag to_digest primary_from primary_to; do
  ...
  files="$(flake_grep -rlF --include='*.nix' -- "$from_digest" "$FLAKE" || true)"
  ...
  if [ -z "$files" ]; then
    ...
    # assuming it.
    if flake_grep -rqF --include='*.nix' -- "$to_digest" "$FLAKE"; then
      continue
    fi
  ...
  # unique in the file.
  flake_sed -i "s|$(esc "$from_digest")|$to_digest|g" "$files"

  if [ "$to_tag" != "$from_tag" ]; then
    if flake_grep -qF -- ":$from_tag@$to_digest" "$files"; then
      # The literal case: `repo:6.3.0-ls312@sha256:…`.
      flake_sed -i "s|:$(esc "$from_tag")@$(esc "$to_digest")|:$to_tag@$to_digest|g" "$files"
    elif flake_grep -qE "= \"$(esc "$primary_from")\";" "$files"; then
      ...
      # its own primary.
      flake_sed -i "s|= \"$(esc "$primary_from")\";|= \"$primary_to\";|" "$files"
    elif flake_grep -qE "= \"$(esc "$primary_to")\";" "$files"; then
...
while read -r to_digest; do
  ...
  flake_grep -rqF --include='*.nix' -- "$to_digest" "$FLAKE" ||
...
#
# The log is in the container's directory: emptied and appended as the
# operator (host/lib.sh), born theirs, never chowned by name.
...
if [ -n "$bad" ]; then
  log_line "$LOGFILE" "verification failed for:$bad"
  rollback
