...
#
# Both the NAME and the ID of each image. The name is what the pin says and
# drives the `<LANG>_VERSION` lookup below; the ID is what gets inspected. A
# container keeps running its image after the tag has moved on to a newer
# build and the weekly prune has removed that build — the tag goes with it,
# and `image inspect <name>` then fails on a container that is perfectly
# healthy. The ID survives as long as the container does. This exact case
# failed every run for two days (litellm-pgvector, 2026-09-07) and emailed
# each one.
list=$(podman_ ps --format '{{.Names}}|{{.Image}}|{{.ImageID}}' || true)
...

{
  ...
  while IFS='|' read -r name image imageId; do
    ...
    # Older podman prints no ImageID column; fall back to the name.
    [ -n "$imageId" ] || imageId="$image"
    ...
    # container".
    labels=$(podman_ image inspect "$imageId" --format '{{json .Labels}}' 2>/dev/null || true)
    case "$labels" in
    "" | null) labels='{}' ;;
    esac
    ...
    #
    # Guarded like `labels` above: with pipefail, an inspect failure here would
    # otherwise become the script's exit status and fail the whole run.
    configVersion=$(podman_ image inspect "$imageId" \
      --format "{{range .Config.Env}}{{println .}}{{end}}" 2>/dev/null |
      sed -n "s/^${envvar}=//p" | head -n 1 || true)
...
# Built in root's own /tmp, published into $OUT_DIR — an operator-owned
# directory, so the publish runs as the operator (host/lib.sh's rule: root
# never chmods, chowns or renames by name where someone else can plant a
# link). Still temp-beside-target and a rename: daedalus reads this on render
# and would otherwise be able to see a partial file.
write_json_atomic "$OUT_DIR/labels.json" <"$tmp"
...
#
# Counted from the private copy just published, not read back out of $OUT_DIR
# as root.
total=$("$JQ" 'length' "$tmp")
labelled=$("$JQ" '[.[] | select(.version != null)] | length' "$tmp")
