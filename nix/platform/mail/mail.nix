...
let
  ...
  notifyEmail = pkgs.writeShellScript "notify-email" ''
    ...
    unit="$1"
    {
      echo "From: ${sender}"
      echo "To: ${alertTo}"
      echo "Subject: [${config.networking.hostName}] FAILED: $unit"
      echo
      echo "Unit $unit entered a failed state on ${config.networking.hostName} at $(${pkgs.coreutils}/bin/date)."
      echo
...
in
{
  ...
  options.fleet.mail = {
    ...
    passwordSopsFile = lib.mkOption {
      type = lib.types.path;
      example = lib.literalExpression "./sops/smtp-app-password.sops";
      description = ''
        The sops-encrypted (binary format) password of the relay account.
        The host's file, handed in. No default — msmtp and every stack that
        sends its own mail read the decrypted secret.
      '';
    };
  };
  ...
  config = {
    sops.secrets."mail-relay-password" = {
      sopsFile = config.fleet.mail.passwordSopsFile;
      format = "binary";
