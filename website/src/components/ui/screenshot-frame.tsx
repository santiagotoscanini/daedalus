...
/** The screenshot slot: a 16:10 frame with quiet fake window chrome. Until a
...
export function ScreenshotFrame({ view, src, alt }: { view: string; src?: string; alt?: string }) {
  return (
    <figure className="card overflow-hidden rounded-2xl shadow-[0_40px_100px_-30px_rgba(0,0,0,0.9),0_0_140px_-60px_rgba(226,121,90,0.35)]">
      ...
      <div className="relative flex h-10 items-center border-b border-hairline bg-white/[0.015] px-4">
        ...
        <figcaption className="absolute inset-x-0 text-center font-mono text-[11px] text-dim">
          daedalus — {view}
        </figcaption>
      </div>
      <div className="relative aspect-[16/10] bg-panel">
        {src ? (
          <img
            src={src}
            alt={alt ?? `The ${view} view in daedalus`}
            className="size-full object-cover"
          />
        ) : (
          <>
            <MockView view={view} />
            <span className="absolute bottom-3 right-4 font-mono text-[9px] uppercase tracking-[0.18em] text-dim">
              preview
            </span>
          </>
        )}
